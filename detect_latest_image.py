...
# Camera to PNG file mapping
camera_to_png = {
    "Putrajaya1": "dkNBK3RJb1V0TDhGTVZSZ3BCSmVndz09.jpg",
    "Putrajaya2": "K24xQnFFSUo1QUszYXA5UWo2QkxVdz09.jpg",
    "Ecopark DS3": "ZjBWK2lsZVZ6bWJjTXEvQ3BwUFc2UT09.jpg",
    "UTM (RP-1)": "Q052THJuWUEycG04K0Z5cHJNekgrUT09.jpg",
    "Ecopark DS1": "UEx4eUlyU3ZBWm1ualI5M0hjUlA4dz09.jpg",
    "Ecopark DS2": "UDRlZFJUd2hDZTllc3hoSnJPU0lBZz09.jpg",
    "GH3 Agrotani": "SitENHBZd1F3eHNSV3R3MnRJVUhnQT09.jpg",
    "FGV GH1 C0019": "eXFUc09BYU5uaTgweEphUGQyOEloQT09.jpg",
    "FGV GH1 C0020": "d09hd0liVFNuMFh6RHZ1eVQvd0U3Zz09.jpg",
    "FGV GH1 C0021": "S1JHWHN5a0FReUEwMFZxRVlVb1g1QT09.jpg",
    "FGV GH1 C0022": "NlJkaVBoT3ZPTW5tMHBNSEg1elJ3UT09.jpg",
    "FGV GH1 C0023": "MnVuT3MvL1RoNzVXbmVreC9XK0ZJQT09.jpg",
    "FGV GH1 C0024": "QjBmTDU5UitTeFRRL0FqNHJCQTJ4Zz09.jpg",
    "UTM Pagoh": "Q0k0RnkxSTR6cGNIVTMvMFpJaUJXdz09.jpg"
}
...
def download_latest_image(camera_name, base_url, save_path='inference/images/latest.jpg'):
    if camera_name not in camera_to_png:
        raise Exception(f"Camera name {camera_name} not found in the camera list.")
    
    image_url = f"{base_url}{camera_to_png[camera_name]}"
    ...
    if img_response.status_code == 200:
        ...
        print(f"Latest image downloaded to {save_path}")
    else:
        raise Exception(f"Failed to download image from {image_url}. Status code: {img_response.status_code}")
...
def detect(opt, save_img=False):
    ...
    week_names = [Path(p).stem for p in source_paths]
    ...
    plt.title('Detected Items from YOLOv7 for Putrajaya1 -first season of cultivation 2022 Oct- Jan 2023')
    plt.xlabel('Weeks')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, default='inference/images/latest.jpg', help='source')
    ...
    parser.add_argument('--end-date', type=str, required=True, help='end date of the season (YYYY-MM-DD)')
    parser.add_argument('--url', type=str, required=True, help='URL to scrape for the latest image')
    parser.add_argument('--camera-name', type=str, required=True, help='Camera name to find the correct image')
    parser.add_argument('--server-url', type=str, required=True, help='Server URL to send the detection results')

    opt = parser.parse_args()
    print(opt)

    download_latest_image(opt.camera_name, opt.url, save_path=opt.source)

    with torch.no_grad():
        class_counters, week_names = detect(opt, save_img=True)

    # Define current_date as the current date
    current_date = datetime.now().strftime("%Y-%m-%d")

    # Calculate the week based on the current date and start date
    week_number = calculate_week(opt.start_date, current_date)

    # Get detection counts for the current week
    try:
        attributes = {
            'health_leaf': sum(class_counters['healthy leaf'].values()),
            'wilt_leaf': sum(class_counters['wilt leaf'].values()),
            'flower': sum(class_counters['flower'].values()),
            'fruit': sum(class_counters['fruit'].values()),
            'week': week_number
        }
    ...
    rockmelon_simulator = ctrl.ControlSystemSimulation(control_system)
    ...
    results = {
        'class_counters': {key: dict(value) for key, value in class_counters.items()},
...
image_path = os.path.join(opt.output, f'{week_names[-1]}_{Path(opt.source).stem}.jpg')  # Path to the latest detected image

# Add the image path to the results
results['image_path'] = image_path

# Create the URL for uploading
urlupload = opt.server_url + "&rpi=1&info=" + json.dumps(results)
...
if r.status_code == 200:
    ...
    print(f"Failed to send results to the server. Status code: {r.status_code}")
